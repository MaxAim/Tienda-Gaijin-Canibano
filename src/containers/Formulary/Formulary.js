...
function Formulary({ setUserInfo, userInfo, completeOrder, display }) {
  ...
  const emailRules =
    userInfo["email"].lastIndexOf("@") < userInfo["email"].lastIndexOf(".") &&
  ...
  const errorHandler = () => {
    emailRules === true
      ? setErrorText("Datos faltantes")
      : setErrorText("Correo electronico invalido");
    handleShow();
  };
  ...
  return (
    <div
      id={"formulary"}
      className={"tableDark"}
      style={{ flexFlow: "column", display: display === 0 ? "none" : "block" }}
    >
      <fieldset>
